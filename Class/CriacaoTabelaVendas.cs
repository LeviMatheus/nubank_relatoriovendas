...
public static class CriacaoTabelaVendas
{
    public static DataTable CriarTabelaVendas(string Str_Vendas){
        ...
        // Regex
        string padrao = @"(?<invoice>\S+)\s+(?<date>\d{2}[\/\-][a-zA-Z]{3}[\-\/]\d{2,4}|\d{2}[\/\-]\d{2}[\/\-]\d{4}|0000-00-00)\s+(?<postal>\S+)\s+(?<vendor>\S+)\s+(?<item>(?:[A-Za-z]+\s*)+?)\s+(?<qty>\d+)\s+(?<unitCost>[0-9.,]+)\s+(?<unitCurr>[A-Z]+)\s+(?<totalPrice>[0-9.,]+)\s+(?<totalCurr>[A-Z]+)";
